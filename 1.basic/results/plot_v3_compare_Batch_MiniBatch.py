...
def plot_n_hids_lrs(data_sets1, data_sets2, outputdir):
    ...
    for data_set1, data_set2 in zip(data_sets1, data_sets2):
        ...
        for data, color in zip(data_set2, colors[len(data_set1):]):
            lbl = "Mini-batch $\\eta$:{}, ".format(data['mc'])
            n_epochs = len(data['ce_train'])
            epochs = [ep for ep in range(0, n_epochs)]
            ax1.plot(epochs, data['ce_train'], '-', color=color, label=lbl+"$Validation$")
            ax2.plot(epochs, data['acc_train'], '-', color=color)
        ...
        # Adjust axis
        ax1.legend(loc='upper right', bbox_to_anchor=(1.6, 1.03),prop={'size':10})
        ax1.set_xlim([0, min_epochs])
        ax2.set_xlim([0, min_epochs])
        ax2.set_ylim([0, 1])
        ax2.yaxis.set_ticks(np.arange(0.0, 1.0, 0.1))
        ax2.yaxis.grid()
        ax1.set_position([0.124, 0.60, 0.775*0.7, 0.33])
        ax2.set_position([0.124, 0.10, 0.775*0.7, 0.33])

        # Output figure to file
        output_file = os.path.join(\
            outputdir, 
            "plot_v3_compare_Batch_MiniBatch_n_hid_{}_lr_{}".format(n_hid, lr))
