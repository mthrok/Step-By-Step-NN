...
def plot_n_hids_lrs(data_sets1, data_sets2, data_sets3, outputdir):
    ...
    for data_set1, data_set2, data_set3 in zip(data_sets1, data_sets2, data_sets3):
        ...
        ax1.set_title(\
            "Cross Entropy for $N_{{hid}}$:{}, $\\alpha$:{}, NAG".format(n_hid, lr))
        ax2.set_xlabel("Epoch")
        ax2.set_ylabel("Accuracy")
        ax2.set_title(\
            "Accuracy for $N_{{hid}}$:{}, $\\alpha$:{}, NAG".format(n_hid, lr))
        min_epochs = np.inf

        # Generate colors
        colors = vary_colors(len(data_set1)+len(data_set2)+len(data_set3))

        # Plot data1
        for data, color in zip(data_set1, colors[:len(data_set1)]):
            lbl = "$sigmoid, \\ \\eta$:{}, ".format(data['mc'])
        ...
        # Plot data3
        for data, color in zip(data_set3, colors[len(data_set1)+len(data_set2):]):
            lbl = "$ReL, \\ \\eta$:{}, ".format(data['mc'])
            n_epochs = len(data['ce_valid'])
            epochs = [ep for ep in range(0, n_epochs)]
            ax1.plot(epochs, data['ce_valid'], '-', color=color, label=lbl+"$Validation$")
            ax2.plot(epochs, data['acc_valid'], '-', color=color)
            min_epochs = n_epochs if min_epochs>n_epochs else min_epochs
        ...
        ax2.yaxis.grid()
        ax1.set_position([0.124, 0.60, 0.775*0.75, 0.33])
        ax2.set_position([0.124, 0.10, 0.775*0.75, 0.33])

        # Output figure to file
        output_file = os.path.join(\
            outputdir, "plot_v4.2_compare_sigmoid_tanh_ReL_n_hid_{}_lr_{}".format(n_hid, lr))
            ...
            try:
                for mc in [0.3, 0.6, 0.9]:
                    file_path = os.path.join(\
                        input_dir, "n_hid_{}_lr_{}_nag_{}_mb.npz".format(n_hid, lr, mc))
                    data1.append(np.load(file_path))

                    file_path = os.path.join(\
                        input_dir, "n_hid_{}_lr_{}_nag_{}_mb_tanh.npz".format(n_hid, lr, mc))
                    data2.append(np.load(file_path))

                    file_path = os.path.join(\
                        input_dir, "n_hid_{}_lr_{}_nag_{}_mb_ReL.npz".format(n_hid, lr, mc))
