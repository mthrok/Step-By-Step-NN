...
def train_network(X_train, T_train, X_valid, T_valid, X_test, T_test, W1, B1, 
    ...
    Returns:
      ...
      act_train(list of float) : The history of the ratio of unit activated during training
      act_valid(list of float) : The history of the ratio of unit activated during validation
      act_test(list of float)  : The history of the ratio of unit activated during testing
    """
    ...
    # Train the network
    ce_valid, acc_valid, ce_test, acc_test, act_train, act_valid, act_test = \
        ...
        for epoch in range(1, n_epochs+1):
            ...
            for batch in range(n_batches):
                ...
                for i, (x, t) in enumerate(zip(X_batch, T_batch), start=1):
                    ...
                    ### Check if each unit is activated at least once 
                    act0 = np.logical_or(act0, x_out)
            ...
            act0 = np.count_nonzero(act0) / n_hid
            
            # Evaluate error on validation data
            ce1, acc1, act1 = 0.0, 0.0, False
            for i, (x, t) in enumerate(zip(X_valid, T_valid), start=1):
                ...
                if np.max(y_out)==np.max(y_out*t):
                    acc1 += 1 / n_valid
            act1 = np.count_nonzero(act1) / n_hid

            # Evaluate error on test data
            ce2, acc2, act2 = 0.0, 0.0, False
...
def main():
    ...
    parser.add_argument('-o', '--output-file-path', type=str, 
                        default=time.strftime("bp_v4.2_%Y%m%d-%H%M%S")+".npz")
    ...
    if input_path is not None:
        ...
        try:
            ...
            act_train = data['act_train'].tolist()
            act_valid = data['act_valid'].tolist()
            act_test = data['act_test'].tolist()
    ...
    W1, B1, W2, B2, dW1, dB1, dW2, dB2, \
        ...
        act_train_new, act_valid_new, act_test_new = \
    ...
    act_train.extend(act_train_new)
    act_valid.extend(act_valid_new)
    act_test.extend(act_test_new)
    ...
    np.savez(output_path, 
             ...
             act_train=act_train, act_valid=act_valid, act_test=act_test)
    print("Saved to {}\n\n".format(output_path))
